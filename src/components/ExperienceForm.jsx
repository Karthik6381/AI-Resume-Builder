                    ...
                    <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                            ...
                            <input value={experience.company || ""} onChange={(e)=>updateExperience(index, "company", e.target.value)} type='text' placeholder='company Name' className='px-3 py-2 text-sm rounded-lg'/>
                             ...
                             <input value={experience.position || ""} onChange={(e)=>updateExperience(index, "position", e.target.value)} type='text' placeholder='Job Title' className='px-3 py-2 text-sm rounded-lg'/>
                              ...
                              <input value={experience.start_date || ""} onChange={(e)=>updateExperience(index, "start_date", e.target.value)} type='month' className='px-3 py-2 text-sm rounded-lg'/>
                               ...
                               <input value={experience.end_date || ""} onChange={(e)=>updateExperience(index, "end_date", e.target.value)} type='month' disabled={experience.is_current} className='px-3 py-2 text-sm rounded-lg rounded-lg disabled:bg-gray-100'/>
                        ...
                        <div className='space-y-2'>
                            <div className='flex items-center justify-between'>
                                ...
                                <button className='flex item-center gap-1 px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors disabled:opacity-50'>
                                    <Sparkles className='w-3 h-3'/>
                                    Enhance with AI
                                </button>
                            </div>
                            <textarea value={experience.description || ""} onChange={(e)=> updateExperience(index, "description", e.target.value)} rows={4} className='w-full text-sm px-3 py-2 rounded-lg resize-none' placeholder='Describle your key responsibilities and achievements...'/>
